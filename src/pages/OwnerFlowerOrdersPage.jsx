...
//
// ✅ Uses shared owner web fetch (absolute backend base)
...
// ✅ profileKey resolve: route -> state -> localStorage (matches other pages)

import React, { useCallback, useEffect, useMemo, useState } from "react";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { getProfileByKey } from "../services/profileRegistry";
import { ownerFetchWeb } from "../utils/ownerApi.web";
...
export default function OwnerFlowerOrdersPage() {
  ...
  const storedKey = normalizeProfileKey(localStorage.getItem("profileKey"));
  const profileKey = routeKey || stateKey || storedKey || "";
  ...
  const loadOrders = useCallback(
    async (mode = "load") => {
      if (!profileKey) {
        ...
        setLoading(false);
        setRefreshing(false);
        setError("Missing profileKey. Open: /world/:profileKey/owner/flowerorders");
        return;
      }
  ...
  const onBack = useCallback(() => {
    ...
    if (profileKey) return navigate(`/world/${encodeURIComponent(profileKey)}/owner/home`, { replace: true, state: { profileKey } });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        ...
        <button className="btnPill" onClick={() => loadOrders("refresh")} disabled={!canUseApi || refreshing} title="Refresh">
      ...
      {loading ? (
        ...
        <div style={styles.list}>
          {orders.map((item) => {
            ...
            return (
              <button key={id} className="card" onClick={() => setExpandedId((prev) => (prev === id ? "" : id))}>
                <div style={styles.cardTop}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <span style={{ ...styles.statusPill, background: pill.bg, borderColor: pill.bd }}>{st}</span>
...
const styles = {
  ...
  loading: { marginTop: 60, display: "flex", alignItems: "center", justifyContent: "center", gap: 12 },
  empty: { marginTop: 80, display: "flex", flexDirection: "column", alignItems: "center" },
...
function css(accent) {
  ...
  *{ box-sizing:border-box; }
  ...
  .btnIcon:hover{ border-color: rgba(255,255,255,0.26); }
  ...
  .btnPill:hover{ border-color: rgba(129,140,248,0.55); }
  ...
  .btnGreen:hover{ border-color: rgba(46,204,113,0.60); }
  ...
  .btnOutline:hover{ border-color: rgba(255,255,255,0.38); }
  ...
  .btnRed:hover{ border-color: rgba(231,76,60,0.62); }
